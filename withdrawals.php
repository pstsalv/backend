...
$fortoday =  mysqli_query($conn, "SELECT id, date, DATE_FORMAT(date, '%Y-%m') AS dday FROM withdrawals WHERE userid='$me' AND ipaddress!='noned' GROUP BY dday ORDER BY id DESC") OR die(mysqli_error($conn));
...
<?php
$check =mysqli_query($conn, "SELECT * FROM withdrawals WHERE userid='$me' AND ipaddress!='noned' AND DATE_FORMAT(date, '%Y-%m')='$transday[dday]' GROUP BY payref ORDER BY id DESC");
