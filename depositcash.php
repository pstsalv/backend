...
$sent = mysqli_query($conn, "INSERT INTO payment VALUES(NULL,'$me','$amount','uncleared','$prevamt','$staffid','no',now(),'$method','$notes','$property','$payref','$user[state]','$user[planid]','$user[agent]','$propdet[canpay_installment]','$user[account_no]','$user[date]','$propdet[type]','$user[branchid]','$user[second_agent]','','','','')") or die(mysqli_error($conn));
$newid =  mysqli_insert_id($conn);

if($sent){
	//customer
	mysqli_query($conn, "UPDATE users SET uncleared=uncleared+$amount WHERE id='$me'");
	//agent
...
//customer
$noty = mysqli_query($conn, "INSERT INTO notification VALUES(NULL,'$me','notification','Cash Deposit','$message','unread',now())");
//agent
