...
$dpixx = mysqli_real_escape_string($conn,$_POST['oldpix']);
...
if(mysqli_num_rows($check)>0){
	...
	$checkbankk = mysqli_query($conn, "SELECT id FROM banks WHERE acc_email='$email'");
						if(mysqli_num_rows($checkbankk)>0){
							
mysqli_query($conn, "UPDATE users SET fname='$fname', state='$state',region='$lga',address='$address',dob='$dob',gender='$gender',pix='$dimage' WHERE id='$me'") or die(mysqli_error($conn));
						}else{}
mysqli_query($conn, "UPDATE users SET fname='$fname', lname='$lname', email='$email', state='$state',region='$lga',address='$address',dob='$dob',gender='$gender',pix='$dimage' WHERE id='$me'") or die(mysqli_error($conn));
