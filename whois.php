...
$check = mysqli_query($conn, "SELECT * FROM users WHERE leftover_batch2='$duid' AND leftover_batch2!=''");
if(mysqli_num_rows($check)>0){
	$user = mysqli_fetch_array($check);
	mysqli_query($conn,"UPDATE users SET leftover_batch2='' WHERE id='$user[0]'");
...
$response = curl_exec($curl);
	...
	<h1>Your account is now reactivated, You can continue with your withdrawal</h1>
<?php
}
?>