...
$fortoday =  mysqli_query($conn, "SELECT id, date_paid, DATE_FORMAT(date_paid, '%Y-%m-%d') AS dday FROM payment WHERE userid='$me' AND status='approved' GROUP BY dday ORDER BY id DESC") OR die(mysqli_error($conn));
...
<?php
$check =mysqli_query($conn, "SELECT id,userid,amount,status,prev_amt,paid_by,admin_approved,date_paid,payment_method,notes,paidfor,payref,state,zoneid,agentid,propert_type FROM payment WHERE userid='$me' AND status='approved' AND DATE_FORMAT(date_paid, '%Y-%m-%d')='$transday[dday]' ORDER BY id DESC");
                            ...
                            <a class="tf-trading-history" href="/pay-detail/<?php print $payment['id'];?>">
                                <div class="inner-left">
									...
									<div class="thumb" style="object-fit:cover">
                                        <img src="<?php print $kaylink;?>/pix/checked.png" alt="image" style="object-fit:cover;padding:6px" />
                                    </div>
                                    <div class="content">
