...
define('PAYSTACK_SECRET_KEY', 'sk_live_7303e6ad47bf4ef9d00c598b7a3091ddbb4ca8c0');
...
if(mysqli_num_rows($checkiffirstpay)>0){
		...
		if($mypropdet['type']=="short"){
$hsetype = "short term payment plan";
}else{
$hsetype = "long term payment plan";
}
		$messagen = "Your $hsetype of N$damt for #$mypropdet[propuid] with Bliss Legacy Limited is successful. Check the app for more details. Regards.";
}else{
...
$checkagent = mysqli_query($conn, "SELECT * FROM users WHERE userid='$user[agent]'");
		...
		if($transamt>=$payamt){
					...
					//create bonus history
					$postit = mysqli_query($conn,"INSERT INTO bonuses VALUES(NULL,'$myagent[id]','$tenperce','Referral bonus from $user[fname] promo payment','approved',now(),'bonus','add','$monthy','$myagent[account_type]','')");
...
$seveno = number_format($sevenperce);
					...
					//create bonus history
					$postit = mysqli_query($conn,"INSERT INTO bonuses VALUES(NULL,'$myagent[id]','$sevenperce','Referral bonus from $user[fname] promo payment','approved',now(),'bonus','add','$monthy','$myagent[account_type]','')");
	...
	if($user['account_type']=="Admin Staff"){
		...
		if($user['agent']!=""){
				...
				if(mysqli_num_rows($checkagent)>0){
					...
					//create bonus history
					$postit = mysqli_query($conn,"INSERT INTO bonuses VALUES(NULL,'$myagent[id]','$outrighttenperc','Referral bonus from $user[fname] outright payment','approved',now(),'bonus','add','$monthy','$myagent[account_type]','')");
...
$dbonusdailsec = 0.15*$outrighttenperc;
$dbonus2dailysec = number_format($dbonusdailsec);
