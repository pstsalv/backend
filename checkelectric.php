...
if (curl_errno($ch)) {
	...
	//echo $response;
if($decodedResponse['code']=="000"){
if(isset($decodedResponse['content']['error'])){
echo '{"status":"failed","message":"'.$decodedResponse['content']['error'].'"}';
}else{
   echo '{"status":"Open","customer_name":"'.$decodedResponse['content']['Customer_Name'].'","address":"'.$decodedResponse['content']['Address'].'"}';
}
}elseif($decodedResponse['content']['Status']=="SUSPENDED"){
echo '{"customer_name":"'.$decodedResponse['content']['Customer_Name'].'","status":"Open"}';
}else{
	echo '{"status":"failed","message":"Network Issue"}';
}
}
