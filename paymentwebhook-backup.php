...
$checkfinbal = mysqli_query($conn,"SELECT SUM(amount) AS totalpay FROM payment WHERE DATE(date_paid) >= DATE_SUB(CURDATE(), INTERVAL 4 DAY) AND status='approved' AND userid='$user[id]' AND paidfor='$myprop[propertyid]' AND propert_type='outright'");
$rowpptdsbw = mysqli_fetch_assoc($checkfinbal);
$sumpptdsbw = $rowpptdsbw['totalpay'];
$totalpaydtdsbw = "$sumpptdsbw";
if($totalpaydtdsbw !==""){
