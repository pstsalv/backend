...
<ul class="menu-tabs kaytabs" style="display:flex">
				...
				<h2 class="mb-3 mt-3">Available Properties</h2> 
...
$checkall = mysqli_query($conn, "SELECT * FROM property WHERE status='Public' AND prop_category='Land' AND (canpay_installment='daily' OR canpay_installment='weekly') AND state='$location' ORDER BY id DESC LIMIT 30");
}else{
	$checkall = mysqli_query($conn, "SELECT * FROM property WHERE status='Public'  AND prop_category='Land' AND (canpay_installment='daily' OR canpay_installment='weekly') AND state='Lagos' ORDER BY id DESC LIMIT 30");
	$alert = '';
}
echo $alert;
...
<?php }?>  
					...
					<div class="tab-gift-item-2 alltab monthly" style="display:none">
					 <?php
		$checkall = mysqli_query($conn, "SELECT * FROM property WHERE status='Public' AND prop_category='Land' AND canpay_installment='monthly' AND state='Lagos' ORDER BY id DESC LIMIT 30");
		if(mysqli_num_rows($checkall)>0){
while($property = mysqli_fetch_array($checkall)){
		?>			
                        <div class="food-box shadow-sm" style="width:100%;margin-bottom:10px">
                            <div class="img-box">
							  ...
							  <img src="<?php print $kaylink;?>/pix/<?php print $property['images'];?>" alt="images" style="height:150px;object-fit:cover">
                                ...
                                <span style="width:auto">₦ <?php print number_format($property['amount']*2);?></span>
                            </div>
                            <div class="content bg-white">
                                <h4><a href="/propertydetails/<?php print $property[0];?>"><?php print ucwords($property['title']);?>..</a></h4>
                                <div class="rating mt-2">
...
<?php }?> 
					...
					<div class="tab-gift-item-3 alltab outright" style="display:none">
					 <?php
		$checkall = mysqli_query($conn, "SELECT * FROM property WHERE status='Public' AND prop_category='Land' AND (canpay_installment='outright' OR canpay_installment='promo') AND state='Lagos' ORDER BY id DESC LIMIT 30");
		if(mysqli_num_rows($checkall)>0){
while($property = mysqli_fetch_array($checkall)){
		?>			
                         <div class="food-box shadow-sm" style="width:100%;margin-bottom:10px">
                            <div class="img-box">
							  ...
							  <img src="<?php print $kaylink;?>/pix/<?php print $property['images'];?>" alt="images" style="height:150px;object-fit:cover">
                                ...
                                <span style="width:auto">₦ <?php if($property['dailyones']!=""){ print number_format($property['amount'],2); }else{ if($property['promocode']=="notapplicable" || $property['promocode']=="applicable"){ print number_format($property['amount'],2); }else{ print number_format($property['amount']*2,2);}}?></span>
                            </div>
                            <div class="content bg-white">
                                <h4><a href="/propertydetails/<?php print $property[0];?>"><?php print ucwords($property['title']);?>..</a></h4>
                                <div class="rating mt-2">
